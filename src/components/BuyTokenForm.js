...
const BuyTokenForm = () => {
  ...
  const ethBalanceNumber = getBalanceNumber(
    new BigNumber(_get(ethBalance, 'data.value', 0n)?.toString()),
    decimals
  );
  ...
  const estimateAmountDest = async () => {
    ...
    const amountBN = await exchangeContract?.read?.getTokenCanBuy([
      parseUnits(amountSource.toString(), 18),
    ]);

    const amountDestValue = getBalanceNumber(
  ...
  const getData = async () => {
    ...
    const tokenBalanceNumber = getBalanceNumber(
      new BigNumber(tokenBalanceBN?.toString()),
      decimals
    );
    const tokenSaleNumber = getBalanceNumber(
      new BigNumber(tokenSaleBN?.toString()),
      decimals
    );
    const totalPresaleNumber = getBalanceNumber(
      new BigNumber(totalPresaleBN?.toString()),
      decimals
    );
    const priceNumber = getBalanceNumber(
      new BigNumber(priceBN?.toString()),
      decimals
    );
  ...
  const handleBuy = async () => {
    try {
      ...
      if (receipt?.status) {
        ...
        toast.success(
          <Success title="Buy Success" txHash={receipt.transactionHash} />,
          {
            icon: false,
  ...
  return (
    <FormStyle>
      <div className="flex justify-center text-[18px] text-white mb-3 font-bold">
        Pre-sale finish in
      </div>
      ...
      <div className="rounded-[8px] p-[12px] text-primary bg-white mb-3">
        ...
        <ProgressBar
          timeWatched={totalSale}
          timeTotal={totalPresale}
        />
        <div className="text-center text-[12px] font-semibold mt-3">
          ...
          <div className="flex items-center gap-1 justify-center">
            <div>YOUR SWAP FOUNDERS NFTs = 0</div>
            <div className="tooltip">
              <img src="images/info-icon.svg" />
              <span className="tooltiptext">
                This NFT grants rights to a percentage of the profits.
              </span>
            </div>
          </div>
        </div>
      ...
      >
        <div className="flex flex-col items-center">
            ...
            <div>
              ...
              <div className="flex justify-between">
                ...
                <div className=" mb-1 text-[14px]">
                  Balance:{' '}
                  {ethBalanceNumber.toLocaleString('en-US', {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 2,
                  }) || 0}
                </div>
              </div>
              <div className="py-2 px-3 rounded-[8px] w-full flex justify-between items-center input-token">
                <input
                  className="text-[24px] bg-transparent font-bold  outline-none"
                  placeholder="0"
                  inputMode="decimal"
                  autoComplete="off"
                  autoCorrect="off"
                ...
                />
                <div className="font-bold text-[14px]">ETH</div>
              </div>
            </div>
          ...
          {account ? (
            <button
              className="uk-button uk-button-medium@m uk-button-gradient  m-0 w-full"
              disabled={loadingBuy || !amountSource}
              onClick={handleBuy}
            >
              {loadingBuy ? (
            ...
            <button
              ...
              onClick={() =>
                dispatch(setAppState({ isOpenConnectWallet: true }))
              }
            >
              Connect Wallet
            </button>
          ...
          <a
            href="#how_to_buy"
            className="font-bold uppercase text-[14px] mt-4 text-primary"
          >
            How to buy?
          </a>
        </div>
